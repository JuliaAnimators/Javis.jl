...
"""
JStar(center, radius; color = "black", npoints=5, ratio=0.5, orientation=0, action=:stroke, vertices = false, reversepath=false) = (args...;center=center, radius=radius, color=color, action=action, npoints=npoints, vertices=vertices) -> _JStar(center, radius, color, npoints, ratio, orientation, action, vertices, reversepath)
